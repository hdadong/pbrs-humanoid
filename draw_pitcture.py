...
def draw_picture(
    ...
    pylab.plot(input1, target1, 'r-', label='nn-pred')
...
leg_position = np.load('/home/bigeast/pbrs-humanoid/leg_position.npy')
for i in range(0, leg_position.shape[1]):
    pred_state_idx = leg_position[:,i]
    true_state_idx = true_position[:,i]
    draw_picture(
                    timestep=0,
                    num_episode=0,
